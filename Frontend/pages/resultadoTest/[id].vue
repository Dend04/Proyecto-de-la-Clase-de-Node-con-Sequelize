<template>
  <div
    class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 pl-20 md:pl-52"
  >
          ...
          <div v-if="testEtiqueta === 'Salud'">
            ...
            <div class="flex items-center">
              ...
              <div class="text-gray-600">
                ...
                <ul class="list-disc ml-6">
                  <li
                    v-for="(valor, nombre) in resultadoDetalles.resultado
                      .valores"
                    :key="nombre"
                  >
                    {{ nombre }}: {{ valor }}
            ...
            <div class="flex items-center">
              ...
              <p class="text-gray-600">
                Deficiencias:
                <span class="font-semibold">{{
                  resultadoDetalles.resultado.deficiencias
                }}</span>
              </p>
            </div>
          </div>
          ...
          <div v-else>
            ...
            <div class="flex items-center">
              ...
              <p class="text-gray-600">
                Respuestas Correctas:
                <span class="font-semibold">{{
                  resultadoDetalles.resultado.respuestasCorrectas
                }}</span>
              </p>
            </div>
            ...
            <div class="flex items-center">
              ...
              <p class="text-gray-600">
                ...
                <span
                  v-if="
                    resultadoDetalles.resultado.respuestasIncorrectas?.length >
                    0
                  "
                >
                  <ul class="list-disc ml-6">
                    <li
                      v-for="(
                        pregunta, index
                      ) in resultadoDetalles.resultado.respuestasIncorrectas.split(
                        ', '
                      )"
                      :key="index"
                    >
            ...
            <div class="flex items-center">
              ...
              <p class="text-gray-600">
                Porcentaje de Aciertos:
                <span class="font-semibold"
                  >{{ resultadoDetalles.resultado.porcentajeCorrectas }}%</span
                >
              </p>
            </div>
...
const fetchResultadoDetalles = async () => {
  try {
    ...
    const testResponse = await $fetch(
      `${config.public.BACKEND_URL}/test/${testId}`
    );
    ...
    const preguntasResponse = await $fetch(
      `${config.public.BACKEND_URL}/preguntas/test/id/${testId}`
    );
    ...
    if (testEtiqueta.value === "Salud") {
      ...
      for (const [idPregunta, valor] of Object.entries(
        response.resultado.valores
      )) {
        const nombreDescriptivo =
          mapeoPreguntas[idPregunta] || `Pregunta ${idPregunta}`;
        valoresMapeados[nombreDescriptivo] = valor;
      }

      // Calcular el estado físico y las deficiencias
      const { estado, deficiencias } =
        determinarEstadoFisicoYDeficiencias(valoresMapeados);
